...
func CraftCopperAndDeposit(cfg *models.Config, char models.Character) error {
	for i := 0; i < 10; i++ {
		...
		maxCopperBar := char.InventoryMaxItems / 8
		maxCopperOre := maxCopperBar * 8

		bankTrans, err := geClient.WithdrawItem(models.BankReq{
			Code:     "copper_ore",
			Quantity: maxCopperOre,
		})
		if err != nil {
			return err
		}
